...
typedef JuneStateBuilder<T extends JuneLifeCycleMixin> = Widget Function(
    T controller);
...
//   T filter<T extends JuneState>(Object Function(T value)? filter) {
//     return Bind.of(this, filter: filter, rebuild: true);
//   }
// }

class JuneBuilder<T extends JuneState> extends StatelessWidget {
...
abstract class Bind<T> extends StatelessWidget {
  ...
  static Bind lazyPut<S>(
    ...
    June.lazyPut<S>(builder, tag: tag, fenix: fenix);
    return _FactoryBind<S>(
      tag: tag,
      // initState: (_) {
      //   onInit?.call();
  ...
  static Future<bool> delete<S>({String? tag, bool force = false}) async =>
      June.delete<S>(tag: tag, force: force);

  static Future<void> deleteAll({bool force = false}) async =>
      June.deleteAll(force: force);

  static void reloadAll({bool force = false}) => June.reloadAll(force: force);

  static void reload<S>({String? tag, String? key, bool force = false}) =>
      June.reload<S>(tag: tag, key: key, force: force);

  static bool isRegistered<S>({String? tag}) => June.isRegistered<S>(tag: tag);
  ...
  static void lazyReplace<P>(BuilderPattern<P> builder,
    ...
    final permanent = (info.isPermanent ?? false);
    delete<P>(tag: tag, force: permanent);
    June.lazyPut(builder, tag: tag, fenix: fenix ?? permanent);
  }
...
class BindElement<T> extends InheritedElement {
  ...
  void _subscribeToController() {
    ...
    if (_needStart == true && localController is JuneLifeCycleMixin) {
...
abstract class Binding extends ConnectionsInterface<List<Bind>> {}
